...
girmx::HttpRequest::HttpRequest(MHD_Connection *connection, const char *url, const char *method, const char *uploadData, size_t *uploadSize, void **cls) :
    m_fulfilled(true),
    m_restricted(true),
...
bool girmx::HttpRequest::match(const char *path) const
{
    return strcmp(m_url, path) == 0;
}


bool girmx::HttpRequest::process(const char *accessToken, bool restricted, girmx::HttpReply &reply)
{
...
const char *girmx::HttpRequest::body() const
{
    return m_body ? m_body->data() : nullptr;
}


int girmx::HttpRequest::end(const HttpReply &reply)
{
    if (reply.buf) {
...
int girmx::HttpRequest::end(int status, MHD_Response *rsp)
{
    if (!rsp) {
...
int girmx::HttpRequest::auth(const char *accessToken)
{
    if (!accessToken) {
        return MHD_HTTP_OK;
    }
